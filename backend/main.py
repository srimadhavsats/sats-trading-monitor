...
# Background task cancellation reference tracker configured safely
...
async def health_diagnostics():
    ...
    try:
        ...
        return {
            ...
            "connected": False,
            "error": "Gateway link connection timed out or dropped",
            "seconds_since_last_tick": seconds_since_last_tick,
...
async def update_whale_threshold(payload: ThresholdUpdateRequestRequest):
    ...
    if not re.match(r"^[A-Z0-9\/]+$", target_symbol):
        raise HTTPException(
            status_code=400, detail="Malformed payload character footprints rejected"
        )
...
async def websocket_endpoint(
    ...
    # Kept flat and outside conditional wrappers to ensure stable execution parameters
